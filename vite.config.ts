 ...
 * when spots are added via SQL. Falls back to the checked-in list if Supabase
 * is unreachable at build time, so a network blip never breaks a deploy.
 */
async function fetchSpotSlugs(env: Record<string, string>): Promise<string[]> {
  ...
  try {
    const res = await fetch(`${url}/rest/v1/spots?select=slug&order=id`, {
    ...
    console.log(`[sitemap] ${rows.length} spot slugs loaded from Supabase`);
